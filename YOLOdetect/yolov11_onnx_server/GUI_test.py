...
class VideoPlayTk:
    ...
    def play_video(self):
        ...
        if not self.is_stopped:
            self.root.after(2, self.play_video)# 需要调整神秘常数
        else:
            self.video_player = None
    # 切换暂停状态的函数
    def video_stop(self):
...
class DetectWindow:
    def __init__(self,root):
        ...
        # 识出来按钮
        self.open_button = tk.Button(self.Page, text='分完啦', command=self.read_message)
    ...
    def send_message(self,cls,is_las,x=320,y=320):
        data_byte = struct.pack('HHBB',x,y,cls,is_las)
        # se.write(data_byte)
        self.state = 0
