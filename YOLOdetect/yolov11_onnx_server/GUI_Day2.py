...
class MainPage:
	def __init__(self, root):
		...
		yolo = YOLO(onnx_path='ReqFile/best.onnx')
...
class VideoPlayTk:
	...
	def play_video(self):
		...
		if not self.is_stopped:
			self.root.after(5, self.play_video)  # 需要调整神秘常数
		else:
			self.video_player = None
...
class DetectWindow:
	def __init__(self, root):
		...
		self.load_label.place(relx=0.7, rely=0.1, relheight=0.2, relwidth=0.3)
		# 创建打开文本
		self.res_text = "识别结果：\n"
		...
		# 识出来按钮
		self.open_button = tk.Button(self.Page, text='compress', command=self.extract)
	...
	def is_move(self, det_obj):
		cnt = 0
		def nms_check(box1, box2, thred=0.8):
		...
		for i in det_obj:
			for j in self.las_det:
				if i['classes'] == j['classes'] and nms_check(i['crop'], j['crop']):
					cnt += 1
		if cnt == len(det_obj) and len(det_obj) == len(self.las_det):
			return True
		return False
	def extract(self):
		data_byte = struct.pack('HHBB', 0, 0, 4, 0)
		se.write(data_byte)
	def send_message(self, cls, is_las, x=320, y=320):
		data_byte = struct.pack('HHBB', int(x), int(y), int(cls), int(is_las))
		# se.write(data_byte)
		print("se.write",data_byte)
		self.state = 0
	...
	def check_detect(self, det_obj):
		if self.state and self.is_move(det_obj):
			...
			if CASE == 1:
				tmp_cnt = [0,0,0,0]
				for i in det_obj:
					cls = self.find_class[i['classes']]
					self.txtline += 1
					self.cls_cnt[cls] += 1
					tmp_cnt [cls] +=1
				cls1 = tmp_cnt.index(max(tmp_cnt))
				cls2 = cls1
				for i in range(4):
					if tmp_cnt[i] > 0 and tmp_cnt[i] <= tmp_cnt[cls2]:
						cls2 = i
				if cls1 != cls2:
					self.txtline+=1
					self.show_txt([self.txtline, cls1, self.cls_cnt[cls1]])
					self.txtline+=1
					self.show_txt([self.txtline, cls2, self.cls_cnt[cls2]])
					self.res_text += "分类完成\n"
					self.res_label.config(text=self.res_text)
				else:
					self.txtline += 1
					...
					self.res_text += "分类完成\n"
					self.res_label.config(text=self.res_text)
				self.send_message(cls1, 1, 320, 320)
			if CASE == 2:
				tmp_cnt = [0,0,0,0]
				for i in det_obj:
					cls = self.find_class[i['classes']]
				...
				if not cls1 == cls2:
					...
					for i in det_obj:
						if i['classes'] == cls2:
							loc = [(i['crop'][0]+i['crop'][2])/2,(i['crop'][1]+i['crop'][3])/2]
					self.send_message(cls2, 0, loc[0], loc[1])
				else:
					self.txtline += 1
					self.show_txt([self.txtline, cls1, self.cls_cnt[cls1]])
					self.res_text += "分类完成\n"
